...
> **윈도우에 아무것도 설치하고 싶지 않다면 → [WEB.md](WEB.md).**
> 서버에서 띄우고 SSH 터널로 브라우저에서 보는 방법입니다. 윈도우 쪽 준비물은
> 브라우저와 (기본 포함된) `ssh` 뿐이고, 앱 기능은 대부분 그대로 동작합니다.
...
- **http://localhost:1420** — 앱 전체를 브라우저에서 씁니다. 로그인하면 독 · 사이드바 ·
  전사록 · 컴포저 · ⌘K 팔레트가 모두 동작하고, 메시지 전송과 실시간 수신도 됩니다.
  로그인: `alice@example.com` / `llack-dev-password`

> 브라우저 모드에서는 로컬 캐시 · 발신 대기열 · OS 알림 · 딥링크가 빠집니다.
> 무엇이 어떻게 다른지는 [WEB.md](WEB.md#브라우저에서-빠지는-것) 에 정리해 두었습니다.
> 그 네 가지를 확인해야 할 때만 2단계로 가세요.
